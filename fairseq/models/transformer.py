...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layers = nn.ModuleList([])
        #print(">>*In TransformerDecoder Z.613 no enc attention: ", no_encoder_attn) ### False
        self.layers.extend(
            [
        ...
        self.num_layers = len(self.layers)
        #print(">>*num TransformerdecoderLayers ", self.num_layers) ### 6
    ...
    def forward(
        ...
        #print(">>>>> IN TrD, features only:", features_only, " -- return_all_hiddens:", return_all_hiddens)
    ...
    def extract_features(
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        #print("\nArgument encoder_out: ", encoder_out)
        ...
        #print(">prev_output tokens in extract features -- only last because of incremental state (Z.739):\n ", prev_output_tokens)

        #print(">In TransformerDecoder, self.embed tokens(prev_output_tokens).shape", self.embed_tokens(prev_output_tokens).shape)

        #print(">self.embed tokens(prev_output_tokens)", self.embed_tokens(prev_output_tokens))
        x = self.embed_scale * self.embed_tokens(prev_output_tokens)
        #print(">in Transformerdecoder: x.shape", x.shape)
        ...
        #####################################################  Inner States, Layers/Hidden states, Embedding - 0th or 7th?  - the output x ist the last
        inner_states: List[Optional[Tensor]] = [x]

        #print(">In TrD encoder_states no layerwise attention for encoder_out, so encoder_out same for all layers")

        #print(">In TrD encoder_out.encoder_out.shape: ", encoder_out.encoder_out.shape)
        #print(">In TrD encoder_out.encoder_padding_mask: ", encoder_out.encoder_padding_mask)
        for idx, layer in enumerate(self.layers):
            encoder_state: Optional[Tensor] = None
            if encoder_out is not None:
                #print("\n\n### encoder_out is not None")
                #print("encoder_out, type & shape: ", type(encoder_out))
                if self.layer_wise_attention:
                    encoder_states = encoder_out.encoder_states
                    assert encoder_states is not None
                    encoder_state = encoder_states[idx]
                else:
                    #print(">In TrD encoder_states no layerwise attention for encoder_out, so encoder_out same for all layers")         ###       YES
        ...
        #print("\n>In TransformerDecoder, extract_features, normalized embeddings in ?inner_states\n", inner_states)
        #print("\n>In TransformerDecoder, extract_features x.shape: ", x.shape)
    ...
    def extract_features_test_posplus(
        ...
        #print("has incremental_state: ", incremental_state)
        #print("prev_output_tokens.shape -- (src): ", prev_output_tokens.shape )
        ...
        if self.layernorm_embedding is not None:
            print("Layernorm is not none") ##### it is None
            x = self.layernorm_embedding(x)
        ...
        #print("\nType x: >>> ", type(x), x.shape )
        return x #, {"attn": [attn], "inner_states": inner_states}



    def extract_features_test_posminus(
        self,
        prev_output_tokens,
        encoder_out: Optional[EncoderOut] = None,
        incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
    ...
    def output_layer(self, features):
        ...
        if self.adaptive_softmax is None:
            ...
            if self.share_input_output_embed:
                #print(">>>>>> output_layer has shared out embed") ############here
                return F.linear(features, self.embed_tokens.weight)
            else:
                #print(">>>>>> output_layer has NOT shared out embed")
                return F.linear(features, self.embed_out)
        else:
            #print(">>>>> output_layer has adaptive_softmax")
            return features
